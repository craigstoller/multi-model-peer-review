...
It's a *personal* Claude Code skill: `SKILL.md` plus one helper script — `roster-agent.js`, which drives the repo-aware roster route. Both live in the `peer-review/` folder, and the install below already copies the whole folder, so there is nothing extra to do.

> **Privacy — and one caveat about what "choosing the document" protects.** A review sends the full text of your doc to OpenAI (Codex) and Google (Gemini) — and, **if `FIREWORKS_API_KEY` is set in your environment**, to Fireworks AI as well, each processed under their respective data policies. For Fireworks, the policy that applies is its zero-data-retention default for open models, which both roster models are. As that policy read on 2026-09-19, prompts and generations are held only in volatile memory for the duration of a request and are not written to persistent storage, token counts and similar metadata are logged, and with prompt caching active, which the repo-aware route's multi-request runs use, some prompt data can stay in volatile memory for several minutes. The 30-day retention described on the same page, and its `store=False` opt-out, belong to Fireworks' Responses API, which this skill never calls: every roster request goes to the chat-completions endpoint ([Fireworks data handling](https://docs.fireworks.ai/guides/security_compliance/data_handling)). That is Fireworks' published policy. This skill cannot verify it, and it can change. The key being set is the whole enablement switch: a key set globally for unrelated Fireworks work turns the roster on. Opt out by unsetting the key for the session or removing the Engine 3 section from your installed copy. Don't peer-review documents with secrets, credentials, or sensitive IP. **To restrict a run, name the engines in the ask** — "review this with Codex only", "skip the roster on this one." The named set is binding and fails loudly: if an engine you named breaks, the skill reports a reduced (even zero-engine) panel rather than silently substituting one you excluded, and the report's panel line says what was excluded by choice versus what failed.
>
> **If you use the `agy` engine, selecting the document is not the whole control.** Its required `read_file(*)` grant lets it read *any* file it asks for, not only the one you named — and it resolves a bare filename by searching the filesystem. So a review could in principle read and quote a file you didn't choose. I tried three narrower grants (`read_file(<path>\*)`, bare `read_file`, forward-slash `**` globs) and agy denied all of them; only the unrestricted wildcard works, so this cannot currently be scoped. **Be clear about what does and does not limit it.** Plan mode blocks *writes* and shell execution, not *reads*, and naming one absolute path in the prompt is an instruction, not an enforced boundary — so a document carrying injected instructions can ask agy to read some other file and quote it back inside the review. Two consequences worth stating plainly: "don't review documents with secrets" doesn't cover this, because the secret is in a *different* file; and anything quoted into a merged report is then sent to every other engine on the next round, turning a local over-read into disclosure to several vendors. **The control that actually works is not using `agy` for documents you didn't author** — and this is **not** agy-specific. Read reach was measured per engine by asking each, from inside this repo, to read a canary file far outside it: **Codex read it** (its `--sandbox read-only` bounds writes, not read paths — verified 2026-08-13), **agy read it** (2026-08-14, as its grant implies), and **the Gemini CLI refused** — its read tool confines reads to the tree you launched in (2026-08-14). **The roster has two routes, and only one of them cannot read files.** The *sealed* route — the original — hands the model the document text inside an API request and gives it no filesystem at all. The *repo-aware* route, added 2026-08-20 and the default for documents you wrote yourself, lets DeepSeek and Kimi read text files inside one repository you name, through a confinement gate; that gate is a bounded check, not a sandbox. So the tiers are: sealed roster (nothing) → repo-aware roster (one repository) → Gemini CLI (bounded to the working tree, though other files in that tree remain reachable) → Codex and agy (whatever you can read). For a document you didn't author, ask for a **sealed roster only** review ("just DeepSeek and Kimi, sealed route — it's a third-party doc"); with no Fireworks key there is no read-safe panel, which is worth knowing rather than papering over.

## What's in this bundle
- `peer-review/SKILL.md` — the skill itself. Drop the whole `peer-review/` folder into your personal skills directory.
- `peer-review/roster-agent.js` — the helper that the repo-aware roster route runs. The same folder copy installs it; nothing invokes it by hand.
...
Sealed roster only on this one — just DeepSeek and Kimi, no repo access (it's a third-party doc)
```
